...
`r fa("palette", fill="blue")` de esta forma aproximamos mejor el hallazgo del cálculo deseado de esta parte controlando por tiempo y por grupo.
...
cont_interp <- tibble(
  consi = c("Pre \\( (t = 1) \\)",
             "Post \\( (t = 2) \\)",
             "Cambio en <br> el tiempo"),
  Treat =c("\\(\\beta_0 + \\beta_2\\)", "\\(\\beta_0+\\beta_1+\\beta_2 +\\beta_3\\)", "\\(\\beta_1 + \\beta_3\\)"),
  Ntreat =c("\\(\\beta_0\\)", "\\(\\beta_0 + \\beta_1\\)", "\\(\\beta_1\\)"),
  diffe = c("\\(\\beta_2\\)", "\\(\\beta_2 + \\beta_3\\)", "Dif en Dif = \\(\\beta_3 !\\)")
) %>% 
  kable(
  escape = F,
  col.names = c(" ", "Tratados <br> \\((D=1)\\)", "No Tratados <br> \\((D=0)\\)", "Diferencia <br> sobre el Tratamiento"),
  align = c("l", "l", "l", "l")
) %>% 
  ...
  column_spec(4, color = "black", italic = F)
...
`r fa("long-arrow-alt-right", fill="red")` El estimador de .hi[diferencias en diferencias] (DID) puede ser obtenido como el **coeficiente** de $t \times d$ en una regresión de MCO
...
`r fa("fighter-jet", fill="blue")` Podemos entonces establecer:
...
`r fa("wrench", fill="red")` El parámetro $\beta_{1}$ es el .hi[impacto] del programa por método de .hi-orange[diferencias en diferencias].
...
`r fa("wrench", fill="red")` Por independencia condicional $\Rightarrow$ $\mathbb{E}(u_{i}|D_{i})=0$, por ende $\beta_{1}$ es un estimador indicado.
...
`r fa("bell", fill="red")` Este método combina información longitudinal, PSM y diferencias en diferencias.
...
`r fa("sketch", fill="blue")` Soporte común:
$$0<P(D=1|X)<1$$
